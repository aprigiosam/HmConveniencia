...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['stats']:
            if cache_type == 'Redis':
                try:
                    ...
                    self.stdout.write(f'Total de comandos: {info.get("total_commands_processed")}')
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'   ✓ 100 operações em {elapsed:.2f}ms ({elapsed/100:.2f}ms/op)')
            )
        ...
        try:
            ...
            if len(result) == 10:
                self.stdout.write(self.style.SUCCESS('   ✓ get_many/set_many funcionando'))
            else:
                self.stdout.write(
